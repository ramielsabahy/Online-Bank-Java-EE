...
public class Withdraw extends HttpServlet {
    ...
    protected synchronized  void doWithdraw (HttpServletRequest request,HttpServletResponse response) throws IOException{
        Date d = new Date();
       ...
       try{ 
            ...
            if(request.getParameter("with")!=""){
                if(Double.parseDouble(request.getParameter("with"))>0){
                u.setWithdraw(Double.parseDouble(request.getParameter("with")));
                }
            }else{
                u.setWithdraw(0);
            }
            double with = u.getWithdraw();
            
            String name=null;
            double balance =0;
            double last = u.getBalance();
            double all = last-with;
            ...
            String updateWithdraw = "update information set withdraw ='"+with+"' where email ='"+u.getName()+"'";
            String insertIntoHistory = "insert into history (withdraw,deposit,transfer,time,mail) values ("+with+","+0+","+0+",'"+d+"','"+u.getName()+"')";
            ...
            st.executeUpdate(updateWithdraw);
...
"        information will be kept safe and secure</p>");
        ...
        out.println("<li id=\"id5\">");
        out.println("<form action=\"Logout\" method=\"get\">");
        out.println("<h5 style=\"margin-top: 18px;padding-left: 50px;margin-left:50px;\">Current Balance is:"+balance+"<button style=\"margin-left:200px\" value=\"signout\" class=\"btn btn-default\">Sign out</button></h5></form>");
        out.println("</li>");
        out.println("</ul></div></div>");
        out.println("</nav>");
        out.println("<div class=\"imag\">");
        out.println("</div>");
        out.println("<h4 style=\"color: black;margin-left: 43%;size:26px\">invalid Balance</h4>");
        out.println("<div class=\"container a\">");
        out.println("<div style=\"width: 100%;height: 50px;\"><h3 style=\"float: left;margin-left: 2%;\">Dear "+name+"</h3></div>");
        out.println("<p>In order to provide Detailed information for your login operations we produced a feature\n" +
"        to let you know the last operation you did.\n" +
