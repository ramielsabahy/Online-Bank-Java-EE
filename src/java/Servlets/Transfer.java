...
public class Transfer extends HttpServlet {
    ...
    protected synchronized void doTransfer (HttpServletRequest request , HttpServletResponse response) throws IOException{
       ...
       try{ 
            ...
            if(request.getParameter("trans")!=""){
                if(Double.parseDouble(request.getParameter("trans"))>0){
            u.setTransfer(Double.parseDouble(request.getParameter("trans")));}
            u.setTo(request.getParameter("to"));
            }else{
                u.setTransfer(0);
            }
            amount = u.getTransfer();
            to=u.getTo();
            String name=null;
            double balance =0;
            double last = u.getBalance();
            double all = last-amount;
            ...
            String updateTransSender = "update information set transfer ='"+amount+"' where email ='"+u.getName()+"'";
            String UpdateTransReceiver = "update information set balance = balance+"+amount+" where card ='"+to+"'";
            String insertIntoHistory = "insert into history (withdraw,deposit,transfer,time,mail) values ("+0+","+0+","+amount+",'"+d+"','"+u.getName()+"')";
            ...
            st.executeUpdate(updateTransSender);
            st.execute(UpdateTransReceiver);
            ...
            while(res.next()){
                ...
                withdraw=res.getString("withdraw");
                trans=res.getString("transfer");
            }
...
"        information will be kept safe and secure</p>");
        ...
        out.println("<div class=\"collapse navbar-collapse\" id=\"bs-example-navbar-collapse-1\">");
        out.println("<ul class=\"nav navbar-nav navbar-right right\">");
        out.println("<li><a href=\"Chat.html\">Chat to Clients</a></li>");
        out.println("<li id=\"id5\">");
        out.println("<form action=\"Logout\" method=\"get\">");
        out.println("<h5 style=\"margin-top: 18px;padding-left: 50px;margin-left:50px;\">Current Balance is:"+balance+"<button style=\"margin-left:200px\" value=\"signout\" class=\"btn btn-default\">Sign out</button></h5>");
        out.println("</li>");
        out.println("</ul></div></div>");
        out.println("</nav>");
        out.println("<div class=\"imag\">");
        out.println("</div>");
        out.println("<h4 style=\"color: black;margin-left: 40%;size:26px\">invalid Balance or Invalid Recipant</h4>");
        out.println("<div class=\"container a\">");
        out.println("<div style=\"width: 100%;height: 50px;\"><h3 style=\"float: left;margin-left: 2%;\">Dear "+name+"</h3></div>");
        out.println("<p>In order to provide Detailed information for your login operations we produced a feature\n" +
"        to let you know the last operation you did.\n" +
